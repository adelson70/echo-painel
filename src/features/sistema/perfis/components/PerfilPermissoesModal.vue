...
import { FindPerfil } from '../services/perfil.service'
...
function mergePermissoes(existing: PermissaoDto[]): PermissaoDto[] {
...
watch(
  ...
  async ([visible, id]) => {
    ...
    try {
      const full = await FindPerfil(id)
      permissoesLocais.value = mergePermissoes(full.permissoes ?? [])
    } catch {
      permissoesLocais.value = defaultPermissoes()
    } finally {
